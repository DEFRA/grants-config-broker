...
```bash
docker build --target development --no-cache --tag grants-config-broker:development .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 grants-config-broker:development
```
...
```bash
docker build --no-cache --tag grants-config-broker .
```

Run:

```bash
docker run -e PORT=3001 -p 3001:3001 grants-config-broker
```
